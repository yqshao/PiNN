...
class GCBlock(tf.keras.layers.Layer):
    """`GCBlock` stands for graph convolution blcok. This block interchanges both scalar properties and equalvariant.
    """
    def __init__(self, pp1_nodes, pi1_nodes, ii1_nodes, pp3_nodes, pi3_nodes, ii3_nodes, **kwargs):
        """
        Args:
            - pp1_layer: pp layer for scalar properties
            - pi1_layer: pi layer for scalar properties
            - ii1_layer: ii layer for scalar properties
            - ip1_layer: ip layer for scalar properties
            - pp3_layer: pp layer for equalvariant properties
            - pi3_layer: pi layer for equalvariant properties
            - ii3_layer: ii layer for equalvariant properties
            - ip3_layer: ip layer for equalvariant properties
        """
        super(GCBlock, self).__init__()
        iiargs = kwargs.copy()
        iiargs.update(use_bias=False)
        self.pp1_layer = FFLayer(pp1_nodes, **kwargs)
        self.pi1_layer = PILayer(pi1_nodes, **kwargs)
        self.ii1_layer = FFLayer(ii1_nodes, **iiargs)
        self.ip1_layer = IPLayer()
        self.pp3_layer = FFLayer(pp3_nodes, **kwargs)
        self.pi3_layer = PILayer(pi3_nodes, **kwargs)
        self.ii3_layer = FFLayer(ii3_nodes, **iiargs)
    ...
    def call(self, tensors):
        ...
        Args:
            tensors (list of tensors): list of [ind_2, prop, basis] tensors
                prop: scalar for scalar and vecs for equalvariant
        """
        ind_2, prop, basis = tensors
        scalar, vecs = prop["scalar"], prop["vecs"]
        # scalr part
        p1 = self.pp1_layer(scalar)
        i1 = self.pi1_layer([ind_2, p1, basis])
        # split i1
        [i1_1, i1_2, i1_3] = tf.split(i1, 3, axis=-1)
        i1 = self.ii1_layer(i1)
        p1 = self.ip1_layer([ind_2, p1, i1])

        # vecs part
        p3 = self.pp3_layer(vecs)
        i3 = self.pi3_layer([ind_2, p3, basis])
        i3 = self.ii3_layer(i3*i1_1)
...
class PiNet(tf.keras.Model):
    ...
    def call(self, tensors):
        ...
        # - `vecs`: equalvarient properties for each atom, with shape `(n_atoms, 3)`;
        ...
        for i in range(self.depth):
            prop = self.gc_blocks[i]([tensors["ind_2"], tensors["p1"], tensors["p3"], basis])
