...
StyleType = Literal["painn", "newton", "general", "simple"]
...
class PIXLayer(tf.keras.layers.Layer):
    ...
    def build(self, shapes):
        ...
        if self.style == "general":
            self.wi = tf.keras.layers.Dense(shapes[1][-1], activation=None, use_bias=False)
            self.wj = tf.keras.layers.Dense(shapes[1][-1], activation=None, use_bias=False)
    ...
    def call(self, tensors):
        ...
        elif self.style == "painn" or self.style == "simple":
            return px_j
        elif self.style == "newton":
            return px_i - px_j
        else:
            raise KeyError(f"Unknown style: {self.style}, should be one of 'painn', 'newton', 'general'")
...
class GCBlock(tf.keras.layers.Layer):
    def __init__(self, style: StyleType, pp_nodes, pi_nodes, ii_nodes, **kwargs):
        ...
        self.pix_layer = PIXLayer(style=style, **kwargs)
